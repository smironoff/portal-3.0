...
// A non-empty question set so the TMCY GeneralFlow renders (content irrelevant to the
// early personal/phone steps this test exercises).
const questions = [
  {
    id: 1,
    question: 'DescribeHighVolatility',
    label: 'describeHighVolatility',
    isMandatory: true,
    answers: [
      { id: 1, answer: 'Risky but can bring high returns', label: 'highRisk', score: 9 },
      { id: 2, answer: 'Safe investment', label: 'safeInvestment', score: 0 },
    ],
  },
  {
    id: 2,
...
test('authenticated TMCY user lands on GeneralFlow and progresses', async ({ page }) => {
  ...
  await page.route('**/nsdata', async (route) => {
    ...
    if (action === 'getLastApplicationsInfo') return route.fulfill(tfboOk(action, [{ applicationId: 1, status: 'INCOMPLETE', portalAccountDomain: 'TMCY', organizationId: 1 }]))
