...
// A non-empty question set so the AU GeneralFlow renders (content irrelevant to the
// early personal/phone steps this test exercises).
const questions = [
  { id: 1, question: 'KOQ', label: 'KOQHoldCFD', isMandatory: true, answers: [{ id: 1, answer: 'a', label: 'a', score: 0 }] },
  { id: 2, question: 'Extra', label: 'extra1', isMandatory: false, answers: [{ id: 2, answer: 'a', label: 'a', score: 9 }] },
]

test('authenticated AU user lands on GeneralFlow and progresses', async ({ page }) => {
  ...
  await page.route('**/nsdata', async (route) => {
    ...
    if (action === 'getLastApplicationsInfo') return route.fulfill(tfboOk(action, [{ applicationId: 1, status: 'INCOMPLETE', portalAccountDomain: 'AU', organizationId: 1 }]))
